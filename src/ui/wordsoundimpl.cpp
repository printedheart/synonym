...
#include "wordsoundimpl.h"

#include <QtNetwork>
#include <QtDebug>

WordSoundImpl::WordSoundImpl(QObject *parent, WordSoundHolder *soundHolder)
    : QObject(parent), WordSoundInterface(soundHolder)
{
...
WordSoundImpl::~WordSoundImpl()
{
}

void WordSoundImpl::findSoundFile(const QString &word)
{
    m_curWord = word;
...
void WordSoundImpl::httpHtmlRequestFinished(int requestId, bool error)
{
    ...
    if (linkIndex != -1) {
        ...
        QIODevice *outDevice = m_soundHolder->getIODeviceForSound(m_curWord, soundName);
        m_httpSound->setHost(url.host());
        m_httpSoundGetId = m_httpSound->get(url.path(), outDevice);
    } else {
...
void WordSoundImpl::httpSoundRequestFinished(int requestId, bool error)
{
    if (requestId != m_httpSoundGetId)
        return;

    if (error) {
        m_soundHolder->setSoundError(m_curWord);
        qDebug() << m_httpSound->errorString();
       
    } else {
        m_soundHolder->setSoundReady(m_curWord);
    }
}


void WordSoundImpl::htmlResponseHeaderReceived(const QHttpResponseHeader &resp)
{
